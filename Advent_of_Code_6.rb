...
def answer_sorter(input)
  input.each do |x|
    sorter = x.split("")
    sorter.delete("\n")
    unique_answers = sorter.uniq
    @sorted_answers.push(unique_answers)
  end
end

def answer_count
  sum = 0
  @sorted_answers.each do |count|
    sum += count.length
  end
  puts "The sum is #{sum}"
end

def people_sorter(input)
  @sorted_people = []
  input.each do |x|
    sorter = x.split("\n")
    @sorted_people.push(sorter)
  end
end
...
def all_questions
  ...
  answer = 0
  @finaltotal = 0
  @sorted_people.each do |people|
    answers = people.map { |string| string.split("")}
    print answers
    answer_count(answers)
    get_group_results(people)
    puts @finaltotal
  end
end
...
def answer_count(answers)
  value = 1
  @answer_counter = Hash.new
  answers.each do |count|
    puts "this is #{count}"
    count.each do |x|
      if @answer_counter.key?("#{x}") == true
        puts true
        @answer_counter["#{x}"] += 1
      else
        @answer_counter["#{x}"] = 1
...
def get_group_results(people)
  @questions_sum = 0
  @answer_counter.each do |key, value|
    if value == people.length
      @questions_sum += 1
    end
  end
  if people.length == 1
    puts "This group has #{people.length} person"
  else
   puts "This group has #{people.length} people"
  end
  puts @answer_counter
  puts "Score: #{@questions_sum}"
  @finaltotal += @questions_sum
end
