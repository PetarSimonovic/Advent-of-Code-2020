@password_entry = "1-7 q: qqqqxvqrkbqqztlqlzq
1-3 q: cqbm
...
def break_up
  @password_entry.each do |line|
    @values, @key, @password = line.chomp.split(" ")
    @key.gsub!(":", "")
    @low, @high = @values.chomp.split("-")
    @password_check << {low: @low.to_i, high: @high.to_i, key: @key, password: @password}
  end
end
...
def analyse_1
  count = 0
  @password_check.each do |line|
    x = line[:password].count(line[:key])
    if x >= line[:low] && x <= line[:high]
      count += 1
    end
  end
  return count
...
def analyse_2
  ...
  @password_check.each do |line|
    ...
    string.each_char.with_index do |chr, index|
      if chr == line[:key] && index + 1 == line[:low]
        hit += 1
      elsif chr == line[:key] && index + 1 == line[:high]
        hit += 1
      end
    end
    if hit == 1
