@input = "light green bags contain 2 pale cyan bags.
...
striped lime bags contain 4 dim orange bags.".gsub!("bags","").gsub!("contain", "").gsub!("bag", "").gsub!(".","").gsub!(",", "").gsub!("  "," ").gsub!("  ", " ").gsub!("no other", "0").split("\n")

@input2 = "light green bags contain 2 pale cyan bags.
dim tan bags contain 3 shiny teal bags, 5 bright white bags, 4 striped bronze bags.
dotted magenta bags contain 2 faded cyan bags, 4 bright yellow bags.
dull silver bags contain 3 striped green bags.
posh purple bags contain 2 mirrored lavender bags, 4 light chartreuse bags, 3 shiny green bags.
...
striped cyan bags contain 4 muted orange bags, 1 light silver bag, 2 pale gold bags, 4 shiny plum bags.
posh violet bags contain 2 dark violet bags, 4 striped olive bags, 1 pale silver bag.
striped lime bags contain 4 dim orange bags.".split("\n")
...
def get_colours
  @valid_bags = []
  get_rules
  @rule_book.each do |rule|
    split_rule(rule)
    direct_check
  end
  until @bag_count == false do
    get_rules
    @rule_book.each do |rule_indirect|
      split_rule(rule_indirect)
      indirect_check(rule_indirect)
    end
    get_bags
  end
end
...
def split_rule(rule)
  ...
  rule.shift
  rule.shift
  @rule_string = rule.join(" ")
end

def direct_check
  if @rule_string.include? @colour
    ind = @rule_string.index("#{@colour}")
    puts "FOUND: #{@container_colour} can hold #{@rule_string[ind-2]} #{@colour}"
    @valid_bags.push(@container_colour)
  end
end

def indirect_check(rule_indirect)
  @valid_bags.each do |validity|
    if @rule_string.include? validity
      ind = @rule_string.index("#{validity}")
      puts "FOUND: #{@container_colour} can hold #{@rule_string[ind-2]} #{@validity}"
      @valid_bags.push(@container_colour)
    end
  end
end
...
def get_bags
 if @bags.empty?
   ...
   old_value = @bags.length
 end
 @bags = @valid_bags.uniq
 puts "There are #{@bags.length} valid bags"
 if old_value == @bags.length
   @bag_count = false
 end
end
...
def string_extract
  extracted = []
  @book_of_bags = []
  @input2.each do |extract|
    extracted = extract.split("bag")
      extracted.each do |cleaner|
        if cleaner.include? "s contain"
          cleaner.gsub!("s contain", "")
        elsif cleaner.include? "s, "
         cleaner.gsub!("s, ", "")
       elsif cleaner.include? ", "
        cleaner.gsub!(", ", "")
      end
    end
    extracted.each do |space_remove|
      space_remove.strip!
      if space_remove == "no other"
      end
    end
    extracted.pop
    @book_of_bags.push(extracted)
  end
end

def get_numbers
  @book_of_bags.each do |bag|
    if bag[0].to_s == @colour
      puts bag
      @duplicates.push(@colour)
      bag_count(bag)
    end
  end
end
...
def bag_count(bag)
  @multiplier.times do
    loop_count = bag.length - 1
    indexer = 1
    loop_count.times do
      display_info(bag)
      get_colour(bag, indexer)
      get_number(bag, indexer)
      puts "New total: #{@bag_total}"
      go_deeper
      indexer += 1
    end
  end
...
def get_colour(bag, indexer)
  col = "#{bag[indexer]}".to_s
  @bag_colour = "#{bag[indexer][2..col.length]}"
  end

def get_number(bag, indexer)
  @val = "#{bag[indexer][0]}".to_i
  puts "#{@bag_colour.upcase} #{@val} added to #{@bag_total}"
  @bag_total += @val
  puts "#{@multiplier} * #{@val} added to #{@bag_total}"
end

def display_info(bag)
  puts "Now checking #{bag[0]}"
  puts "#{@val} - #{@bag_colour}"
  puts "Total so far: #{@bag_total}"
