...
ecl:oth hcl:#602927 eyr:2025 iyr:2013 hgt:151cm byr:1992 pid:812583062".split(/^$\n/)
...
def passport_check(passport_inputs)
  valid = 0
  passport_inputs.each do |passport|
    id_check = true
    @fields.each do |field|
      unless field == "cid:"
        if !passport.include? field
          id_check = false
        end
      end
    end
...
def passport_extract(passport_inputs)
  passport_inputs.each_with_index do |passport|
    passport_extract = Hash.new
    @fields.each do |field|
      if passport.include? field
        passport_extract[field] = passport.split(field)[1].split(" ")[0]
      end
    end
    @passport_scan << passport_extract
  end
end


def passport_check_2
  valid_passports = 0
  @passport_scan.each do |check|
    puts check
    @validity = 0
    check_byr(check)
    check_iyr(check)
...
def check_byr(check)
  if check["byr:"].to_i >= 1920 && check["byr:"].to_i <= 2002
    puts "birth: passed"
    @validity += 1
  else
    puts "birth: failed"
...
def check_iyr(check)
  if check["iyr:"].to_i >= 2010 && check["iyr:"].to_i <= 2020
    puts "issue year: passed"
    @validity += 1
  else
    puts "issue year: failed"
    @validity = 0
  end
end

def check_eyr(check)
  if check["eyr:"].to_i >= 2020 && check["eyr:"].to_i <= 2030
    @validity += 1
    puts "expiry year: passed"
  else
    puts "expiry year: failed"
    @validity = 0
  end
end

def check_height(check)
  if check["hgt:"].include? "cm"
    height = check["hgt:"].gsub("cm", "").to_i
  ...
  elsif check["hgt:"].include? "in"
      height = check["hgt:"].gsub("in", "").to_i
...
def check_hcl(check)
  if check["hcl:"].include? "#"
    hair = "#{check["hcl:"].gsub("#","")}"
    if hair.length == 6
      puts "hair colour: passed length"
    else
      @validity = 0
      puts "hair colour: failed"
    end
    if hair =~ /\W/
      @validity = 0
      puts "hair colour: failed"
...
def check_ecl(check)
  ...
  if eye_colour.include? check["ecl:"]
    @validity += 1
    puts "eye colour: passed"
  else
    puts "eye colour: failed"
    @validity = 0
  end
end
...
def check_pid(check)
  passport = check["pid:"]
  if passport.length == 9 && passport !~ /\D/
    @validity += 1
    puts "passport: passed"
  else
    @validity = 0
    puts "passport: failed"
  end
end

passport_check(passport_inputs)
passport_extract(@valid_passports)
passport_check_2
